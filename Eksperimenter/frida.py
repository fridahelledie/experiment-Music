...
y, sr = librosa.load(reference_audio_path, sr=None, duration=0.5)
...
def online_tw(live_features, ref_features, _D, _P, _t, _j):
    ...
    new_rows = live_features.shape[0] - _D.shape[0]
    ...
    # if t < D.shape[0] and j < D.shape[1] and D[t, j] == np.inf:
    #     D[t, j] = EvaluatePathCost(t, j, live_features, ref_features, D)
...
def simulate_live_audio_input(audio_path, buffer_size, ref):
    live_audio, sr_ = librosa.load(audio_path, sr=sr, duration=0.5)
