...
def GetInc(i, j, X, Y):  # determines the next movement direction, row, column, or both
    ...
    for m in range(0, min(c, i)):
        for n in range(0, min(c, j)):
            # print(f"{i-m},{j-n}")
            path_costs[n, m] = EvaluatePathCost(i-m, j-n, X, Y)
            # print(f"[{n},{m}] = {path_costs[n, m]}")
...
test_audio_path = "audio_file_slowed.wav"
...
for frame_start in range(0, len(test_audio_file), buffer_size):
    ...
    if not audio_queue.empty():
        ...
        # X_np = np.hstack(X)
        #
        # D, wp = dtw(X_np[max(0,i - (c//5)):i], Y[max(0,j - (c//5)):j], metric='euclidean')
        # show_dtw(D, np.array(P))

        # show_path(np.array(P))

P = np.array(P)  # Convert to NumPy array so it can be made into a figure

# Compute chroma features for x
x, x_sr = librosa.load(test_audio_path, sr=None)
X_chroma = librosa.feature.chroma_stft(y=x, sr=x_sr)  # list of fixed features

# Get realtime chromafeatures
X_np = np.hstack(X)
